Shader "Skybox/ProceduralCustom" {
Properties {
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		#if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
			OUT.sunColor	= (cOut * _LightColor0.xyz);
		#endif
		...
		// if y < 0 [eyeRay.y > 0] - sky
			...
			if(y < 0.0)
			{
				...
				col += mie * IN.sunColor * ((sin(y + _Time.w) + 1) / 2);

				float3 a = 3;
				float3 b = col.r + col.b + col.g;
				float test = a - b;
				if (test < 1){
					col = lerp(col, col * test, test);
				}
			}
		#endif
